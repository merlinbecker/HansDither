...
- [X] T005 [US1] Validierung durchgeführt und protokolliert: quickstart.md Szenario 1 auf ECHTER HARDWARE (5s gehaltene Richtungstaste + Malstriche) — **bestanden** (Befund: "keine Aussetzer mehr gemerkt"); der geplante objektive `playdate.getStats()`/Sampler-Vergleich vor/nach dem Fix wurde NICHT durchgeführt (rein subjektiver Spieltest, keine Messwerte) — ehrlich als solcher in `quickstart.md` vermerkt statt fälschlich als "objektiv nachgewiesen" ausgewiesen; Ergebnis fließt in die Risiko-Bewertung R-23 zurück (siehe T019)
...
- [X] T010 [US2] Validierung durchgeführt und protokolliert: quickstart.md Szenario 2 auf echter Hardware (Crank-Gefühl) — **bestanden** (Befund: "geht super"), inkl. Regressionsprüfung der B+Crank-Zoomkette im selben Testlauf
...
- [X] T015 [US3] Validierung durchgeführt und protokolliert: quickstart.md Szenario 3 auf echter Hardware — **bestanden**, inkl. Prüfung, dass `"reset frame"` an keiner Stelle der Oberfläche mehr erscheint
...
- [X] T019 [P] Risiko-Review abgeschlossen: T005 wurde auf echter Hardware durchgeführt — **bestanden** (Befund: "keine Aussetzer mehr gemerkt"), ABER ohne den ursprünglich vorgesehenen objektiven `getStats()`/Sampler-Vergleich (rein subjektiver Spieltest). `arc42/11-risiken-und-technische-schulden.md` R-23 wird daher NICHT als "mitigiert" (objektiv nachgewiesen), sondern ehrlich als **"Akzeptiert (subjektiv bestätigt)"** eingestuft: SC-001 selbst fordert nur eine subjektiv wahrnehmbare Reaktionsfähigkeit, die auf echter Hardware bestätigt wurde; der zusätzlich geplante formale Messnachweis wurde vom Projektinhaber als nicht nötig erachtet und bewusst ausgelassen — Owner: Entwickler

**Checkpoint**: Alle drei Stories unabhängig funktionsfähig, headless-testverifiziert (`lua tests/headless_tests.lua` → ALLE TESTS BESTANDEN, `pdc` fehlerfrei) UND auf echter Hardware validiert (T005/T010/T015, alle bestanden); arc42/ADR-Dokumentation deckt sich nach T016-Korrekturen mit dem Implementierungsstand; Audit-Evidenz-Checkpoints vollständig entschieden (R-23 als "Akzeptiert (subjektiv bestätigt)" statt "Offen", siehe T019)
...
- [X] Risiko-/Schulden-Review: R-24 (Wegfall der Frame-Schutzfunktion) ist **Akzeptiert**, keine weitere Aktion. R-23 (Zoom-Room-Ursachenbefund) ist **Akzeptiert (subjektiv bestätigt)** — Hardware-Test in T005 bestanden ("keine Aussetzer mehr gemerkt"), aber ohne den ursprünglich geplanten objektiven `getStats()`/Sampler-Vergleich; Projektinhaber-Entscheidung, den subjektiven Nachweis als ausreichend zu akzeptieren. Owner: Entwickler.
...
- [X] T005/T010/T015 (manuelle Hardware-Validierung je Story) vom Projektinhaber auf echter Hardware durchgeführt — alle drei **bestanden**; T019 (finale Risiko-Bewertung R-23) entsprechend auf "Akzeptiert (subjektiv bestätigt)" abgeschlossen. Damit sind Code, Tests (headless), Build UND Hardware-Validierung vollständig und grün — keine offenen Punkte mehr in diesem Feature.
