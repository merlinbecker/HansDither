...
- Q: What does the Crank do in Tile View now? → A: **Tile picker.** Turning the Crank (without B) brings up a filmstrip overlay of the tiles actually used in the image; each ~30° of net rotation moves the selection one tile further, wrapping at the end. Landing on tile 1 (white) means "no selection" (toggle mode), matching the eyedropper. *(Tenth Round: the overlay now opens only after a full crank revolution; the ~30°/tile stepping applies once it is open.)*
...
### Session 2026-09-07 (Tenth Round — Tile-Picker: volle Umdrehung zum Öffnen, aus dem Hardware-Test)

- Q: Der Tile-Picker (Kurbel ohne B) erschien schon bei der kleinsten Kurbelbewegung — versehentliches Antippen/Rütteln beim Ein- oder Auspacken der Kurbel ließ das Overlay ständig aufpoppen. Wie verhindern? → A: **Der Picker erscheint erst nach einer VOLLEN Umdrehung (360°).** Der Aktivierungs-Akkumulator ist vorzeichenbehaftet — Vor-/Zurück-Rütteln hebt sich gegen 0 auf und öffnet nichts; eine ganze Drehung in **beliebiger** Richtung öffnet (der Picker hat keinen Kurbel-Indikator, also nicht auf eine Richtung festlegen — analog zur `SelectionRoom`-Sync-Geste bei 720° und zur `PixelRoom`-Rotation bei 360°, aber richtungsneutral). **Ist der Picker offen, wählen kleine Drehungen (~30°/Kachel) wie bisher die Kachel** — man kann also fein auswählen und die Kurbel danach wieder einpacken, ohne das Overlay erneut auszulösen. Nach der 1,5-s-Auto-Ausblendung braucht es erneut eine volle Umdrehung. Die Öffnungsdrehung selbst wählt noch keine Kachel. **B + Kurbel** (Zoom-Kette vorwärts / Frame-Room rückwärts) bleibt unberührt — dort greift weiterhin die Tick-Schwelle.
...
- **FR-025**: Turning the Crank in Tile View **without B held** MUST open a tile-picker overlay (a filmstrip of the tiles actually referenced in the image) and set it as the active drawing tile; each ~30° of net rotation moves the selection one tile further, wrapping at the list ends. The overlay auto-hides ~1.5 s after the last rotation. The filmstrip MUST be drawn **inside the consolidated overlay bar** (FR-028), anchored to the edge opposite the tile cursor — never screen-centred over the artwork *(revised 2026-09-06, from hardware testing)*. **The overlay MUST NOT open until the Crank has been turned a full revolution (≥ 360° net) in one direction** — the activation accumulator is signed, so back-and-forth jiggle cancels toward zero and never opens it; a full turn in **either** direction opens it. The opening revolution itself selects no tile. Once open, the ~30°/tile fine stepping applies as above, so the user can pick and then re-dock the Crank without the overlay re-triggering; after the auto-hide, another full revolution is required to re-open *(revised 2026-09-07, from hardware testing — the picker previously opened on the smallest crank movement)*
...
- **SC-007**: Turning the Crank a **full revolution** in Tile View opens a tile-picker overlay **within the consolidated overlay bar, never covering the tile under the cursor** (a partial turn or back-and-forth jiggle does not open it); once open, further rotation cycles the referenced tiles with wraparound and sets the active drawing tile; the eyedropper shows a brief "Tile N picked" confirmation in the same bar *(revised 2026-09-07)*
