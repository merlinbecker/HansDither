...
| arc42 Kap. 4 — Lösungsstrategie | **Open** | Owner: `/speckit-plan`. Konsolidierte Overlay-Leiste + Frame-Room als Leitentscheidungen aufnehmen. Trigger: Plan-Lauf für diesen Nachtrag |
| arc42 Kap. 5 — Bausteinsicht | **Open** | Owner: `/speckit-plan`. `FrameManagementView` → Room mit `entered()`/Exit-Lifecycle; Tile-View-Overlay-Zeichnung als eine Layout-Einheit. Trigger: Plan-Lauf |
| arc42 Kap. 6 — Laufzeitsicht | **Open** | Owner: `/speckit-plan`. **Doppel-offen**: (a) die seit Spec 010 fehlenden Laufzeitsequenzen (Ebenen-Cyclen, Tile-Picker, Frame-Verwaltung — `tasks.md` T052 vermerkt das selbst) und (b) die neue Room-Eintritt/-Verlassen-Sequenz. Trigger: Plan-Lauf; Re-Eval bei Implementierung |
| arc42 Kap. 7 — Verteilungssicht | **N/A** | Keine Build-/Paketierungsänderung. Trigger: — |
| arc42 Kap. 8 — Querschnittliche Konzepte | **Open** | Owner: `/speckit-plan`. Eingabe-/Overlay-Konzept: eine cursorabgewandte Overlay-Leiste; Room-Gesten „B + Kurbel rückwärts/vorwärts". Trigger: Plan-Lauf |
| arc42 Kap. 9 — Architekturentscheidungen (+ `arc42/adr/`) | **Open** | Owner: `/speckit-plan`. Zwei neue ADRs: „Frame-Verwaltung als persistenter Room", „Konsolidierte Tile-View-Overlay-Leiste". Trigger: Plan-Lauf |
| arc42 Kap. 10 — Qualitätsanforderungen | **Open** | Owner: `/speckit-plan`. Usability-Szenario: Overlay verdeckt nie die Cursor-Zelle (SC-008); Frame-Umsortieren „in Ruhe" ohne Dauer-Tastendruck. Trigger: Plan-Lauf |
| arc42 Kap. 11 — Risiken & technische Schulden | **Open** | Owner: `/speckit-plan`. Risiko: Spec-011-Annahme „Schüttel-Geste in der Frame-Verwaltung inaktiv, weil B gehalten" verfällt mit dem persistenten Room. Trigger: Plan-Lauf; Re-Eval bei Room-Lifecycle-Umbau |
| Secure-Architecture-Preset (iSAQB) | **N/A (bestätigt)** | Rein lokale UI-/Room-Umstrukturierung: kein Netzwerk, keine Secrets, keine Persistenzänderung (Frame-Reihenfolge/-Anzahl über v1.1 unverändert), keine neue Angriffsfläche. Re-Eval-Trigger: falls Overlay-/Room-Zustand je persistiert oder extern konfiguriert wird |
| Constitution V — Gate 1 (`lua tests/headless_tests.lua`) | **Open** | Owner: `/speckit-implement`. `FrameManagementView`-Tests (Navigation/Markieren/Verschieben/Löschen) auf Raster- + Room-Modell umschreiben; Overlay-Anchor-Regel headless prüfen, soweit ohne echtes Rendering möglich. Trigger: erste Code-Änderung |
| Constitution V — Gate 2 (`buildNumber` +1, `pdc`) | **Open** | Owner: `/speckit-implement`. Start-`buildNumber` = aktueller Stand des Branches. Trigger: erster Testbuild dieses Nachtrags |
| Manuelle Simulator-/Hardware-Integration | **Open** | Owner: Merlin. Overlay verdeckt Cursor nie (alle Cursorpositionen, SC-008); Frame-Room Eintritt/Verlassen + Raster-Umsortieren; plus die weiterhin offenen Spec-010-Phase-7-Punkte (T053, T055–T059). Trigger: nach Implementierung |
...
- The `/speckit-plan` decision "the Frame Room registers **no** system menu items" (R12 / draft ADR-047) is **overturned**: the room registers **"delete frame"** and **"duplicate frame"** on `playdate.getSystemMenu()`, exactly as `SelectionRoom` registers "new / copy / delete image". Delete **reuses** the `SelectionRoom` confirm-dialog pattern (`confirmingDelete` / `drawConfirmDeleteDialog`, A = yes / B = no) — reuse of a proven pattern, Constitution IV. **ADR-047 must be written with the menu + confirm-dialog design**, not the menu-less one.
