...
- **Device measurement (R-33, owner Merlin)**: time the `entered()` build for a 12-frame image on hardware. If it exceeds ~1 frame, fall back to lazy per-cell rendering in `drawCell` with a small LRU, or a coarser scale. Decision recorded in ADR-048 after the measurement.
...
- `UndoPrompt.draw()` still drawn last as a separate modal layer (Spec 011 FR-013 requires it stay fully modal — it cannot fold into a passive band). Its centred box already clears a top- or bottom-anchored bar; ADR-049 records the coordination rule so future changes keep it.
