...
- **Perf-Review — Pixel-Verschiebung gepuffert (US1, aus dem Hardware-Test: "das Verschieben ist zu inperformant")** ✅ — Standalone-Messung: ein 1px-Shift kostete ~192.000 `image:sample()`-Aufrufe + 375 `image.new()` PRO Tastendruck (375-Tile-Rebuild + Rehash sofort bei jedem Druck). `LayerModel.shiftLayerContent` in `decodeLayerGrid`/`materializeShiftedGrid`/`imageFromGrid` zerlegt (bleibt als synchroner Wrapper bestehen — bestehende Tests unverändert gültig). `EditorRoom.shiftActiveLayer` dekodiert die Ebene nur einmal je Verschiebe-Geste (`pendingShift`) und akkumuliert weitere Tastendrücke als O(1)-Wrap-Versatz; materialisiert wird erst in `EditorRoom:flushLayerShift()`, an jeder Stelle, die kanonische Tiles braucht (`applyTileEdits`, ZoomRoom `zoomIntoPixelRoom`/`commitAndReturnToEditor`/`commitForTerminate`, neuer `BButtonUp`, defensiv `entered()`/`buildPauseMenuImage()`). `buildZoomContext()` synthetisiert die 3x3-Live-Vorschau während einer offenen Sitzung direkt aus dem Puffer, bleibt also bei jedem Tastendruck aktuell. Offset-Algebra (Modulo ist additiv) beweist: N gepufferte Schritte + 1 Flush == N sofortige Einzelschritte — per Vergleichstest bestätigt. `newMockImage` bekam eine `copy()`-Methode (SDK-Äquivalent, bislang ungenutzt). Durch spec (FR-002-Note, Architecture Governance, Risk Record resolved)/research (R6-Nachtrag) + arc42 Ch.4/5/9 + **ADR-043** gezogen. Vier neue Testabschnitte + Fix der bestehenden `shiftActiveLayer`-Assertion. 393 Assertions grün, buildNumber 28 → 29, pdc sauber.
- **Phase 7** (T044–T059): Gates grün (T044–T048) ✅; arc42 Ch.4/Ch.5/Ch.8/Ch.9 + ADR-039..043 (T049–T052) ✅; T054 (shiftLayerContent-Performance) architektonisch adressiert (ADR-043), Geräte-Messung weiterhin offen; Simulator-Integrationstests (T053, T055–T059) **offen** (Gerät/Simulator).
...
- [X]/[ ] T054 **Architektonisch adressiert (ADR-043, Perf-Review), Geräte-Messung offen.** `LayerModel.shiftLayerContent` baute urspünglich je Tastendruck alle 375 Tiles neu (Standalone-Messung: ~192k `image:sample()`-Aufrufe, laptop-seitig 212 ms — auf Hardware vom Nutzer als "zu inperformant" gemeldet). Behoben durch Aufschieben: `EditorRoom.shiftActiveLayer` dekodiert nur einmal je Verschiebe-Geste, materialisiert (die weiterhin vollen 375 Tiles) erst in `flushLayerShift()` am Gestenende — der **einzelne Flush selbst** ist also weiterhin O(375 Tiles) und muss auf dem Gerät noch gegen 16 ms geprüft werden (Playdate-Simulator/Hardware, nicht die Laptop-Standalone-Messung). "Auf tatsächlich geänderte Tiles beschränken" entfällt strukturell nicht (ein 1px-Shift ändert alle 375), aber N Tastendrücke zahlen den Preis jetzt nur EINMAL statt N-mal.
