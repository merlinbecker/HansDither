...
| FrameManagementView | **Neu (Spec 010, US4).** Aus dem EditorRoom per B + Kurbel rueckwaerts. **Seit Spec 010 8./9. Runde (AD-048):** ein DAUERHAFTER Room (kein „B halten"-Modal mehr) mit `playdate.ui.gridview`-Thumbnail-Raster (3 Spalten, `thumbCache` je Position aus `imageData.frames[f]` + Imagetable). Steuerung wie `SelectionRoom` + Zusatz: D-Pad navigiert das Raster (hoch/runter = ±NUM_COLS), **A markiert/entmarkiert** (reiner Umschalter — loescht NIE), D-Pad bei markiertem Frame verschiebt ihn in der Sequenz (Links/Rechts = ±1, Hoch/Runter = NUM_COLS als sequenzielle Nachbar-Swaps; `frameLayers` + Composite-Cache + `thumbCache` im Gleichschritt, je Schritt ein `EditorRoom:onFramesReindexed({swapped})`). Das **System-Menue** traegt „delete frame" (A/B-Bestaetigungsdialog nach `SelectionRoom.confirmingDelete`-Muster, wirkt auf den Cursor-Frame, abgelehnt bei nur 1 Frame) und „duplicate frame" (tiefe Kopie direkt dahinter → `onFramesReindexed({inserted})`, abgelehnt bei 12). **Verlassen: B + Kurbel VORWAERTS**, scharf erst nachdem B seit `entered()` einmal losgelassen wurde (`bReleasedSinceEnter`) — der Kurbel-Nachlauf der Eintrittsgeste wirft nicht sofort wieder raus (FR-022). Setzt `imageData.returnFrame`; eine Kurbel-API im Room (`getCrankTicks(4)`). Keine Ebenen-Verwaltung (AD-041). **Seit Spec 011:** der `confirmDelete()`-Pfad meldet den geloeschten Frame (tiefe Kopie + Cache-Kopie) **vor** `table.remove` an `EditorRoom:recordDeleteFrame`. Die Schuettel-Geste ist hier **nicht** aktiv (Spec 011 FR-010 schliesst die Frame-Verwaltung aus; der Room startet den Accelerometer nicht) — der Undo-Dialog erscheint erst nach Rueckkehr in den Tile View. |
...
| Bauchbinde | Wiederverwendbare UI-Komponente fuer Hinweisbaender (Frame-Anzeige "Frame n/m", Fehlerstatus). **Seit Spec 010 8. Runde (AD-049):** `draw(lines, side, vAnchor, w, h)` nimmt mehrere Zeilen + einen vertikalen Anker ("top"/"bottom"); `drawBottom(text, side, w, h)` bleibt signaturgleicher Einzeiler-Wrapper (`SelectionRoom` unveraendert). Der `EditorRoom` zieht damit Frame/Ebenen-Label, „Tile N picked"-Toast und Statuszeile in EINE cursorabgewandte Leiste (`overlayAnchor(cursor.y, GRID_ROWS)`); der Tile-Picker-Filmstreifen liegt in derselben Zone statt bildschirmmittig (FR-028 / SC-008). |
