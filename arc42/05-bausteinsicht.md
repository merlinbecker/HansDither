...
| LayerModel | **Neu (Spec 010).** Reine-Tabellen-Datenmodell fuer die feste 3-Ebenen-Struktur je Frame: `newFrameLayersFromFlat`/`padTo3`/`validate`, `compositeToFlat` (flacher Cache), `compositeToTiles` (pixel-genau, noch nicht verdrahtet), `cycleActive`/`clampActive`, `shiftLayerContent` (Pixel-Shift + Retiling, synchroner Einzelschritt-Wrapper). Kein Add/Delete von Ebenen (AD-039). **AD-043 (Perf-Review)**: `shiftLayerContent` in drei Bausteine zerlegt — `decodeLayerGrid` (Pixelraster lesen), `materializeShiftedGrid` (Tiles durch einen Wrap-Versatz gelesen neu bauen), `imageFromGrid` (ein einzelnes Tile fuer die Live-Vorschau synthetisieren) — damit EditorRoom den Decode nur einmal pro Verschiebe-Geste zahlt statt einmal je Tastendruck. |
...
- Spec 010 US1 (AD-043, Perf-Review nach Hardware-Test): `shiftActiveLayer(direction)` (Einstieg von der ZoomRoom, B + Pfeiltaste) baut die 375 Tiles NICHT mehr pro Tastendruck neu — es dekodiert die aktive Ebene einmal je Verschiebe-Geste (`pendingShift.grid`) und akkumuliert weitere Druecke nur als Wrap-Versatz. `flushLayerShift()` materialisiert (baut die 375 Tiles + kompositiert neu) genau einmal, dort, wo kanonische Daten wirklich gebraucht werden (siehe ZoomRoom-Aufrufstellen 5.2.4; zusaetzlich defensiv in `entered()` und `buildPauseMenuImage()`). `buildZoomContext()` liest die 3x3-Vorschau waehrend einer offenen Sitzung direkt aus dem Puffer (`LayerModel.imageFromGrid`) statt aus echten Tiles.
...
- Spec 010 US1 (Perf-Review, AD-043 — dasselbe Cache-statt-Neuberechnung-Muster wie AD-035, hier auf die B+Pfeil-Verschiebe-Geste angewendet): `shiftActiveLayerContent(direction)` ruft `editorRoom:shiftActiveLayer(direction)` — das puffert (siehe 5.2.3), materialisiert also NICHT sofort — und aktualisiert danach trotzdem den 3x3-Kontext ueber `editorRoom:currentZoomContext()`, der die Slot-Bilder jetzt direkt aus dem gepufferten Raster synthetisiert (`LayerModel.imageFromGrid`). `flushShiftAndRefresh()` (neu) buendelt Flush + Kontext-Refresh fuer die drei echten Ausgangspunkte (`zoomIntoPixelRoom`, `commitAndReturnToEditor`, `commitForTerminate`) sowie den neuen `BButtonUp`-Handler; ein Malstrich committet ueber `applyTileEdits`, das intern selbst zuerst flusht.
