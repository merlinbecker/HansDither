...
- Status: umgesetzt (Spec 010, 4. Klarstellungsrunde aus dem Hardware-Test); **Picker-Aktivierung revidiert 10. Runde (2026-09-07)**
- Entscheidung: Im Tile View wechselt **B halten + Hoch/Runter** die aktive Ebene (+1/-1, Wrap 1..3) und **B halten + Links/Rechts** den Frame (Rechts am Ende: neuer Frame als tiefe Kopie). Die freie **Kurbel (ohne B)** oeffnet ein **Tile-Picker-Overlay** — **10. Runde: erst nach einer vollen Umdrehung** (`pickerArmDegrees`, vorzeichenbehaftet, `|·| ≥ 360°`, beliebige Richtung; Vor-/Zurueck-Ruetteln hebt sich gegen 0 auf; die Oeffnungsdrehung waehlt keine Kachel). Ist es offen, geht es je ~30° Netto-Drehung eine Kachel weiter durch die **referenzierten** Tile-Indizes (`referencedTileIndices()` scannt die **Ebenen-Positionen** `frameLayers[*].layers[*].positions`, **nicht** den flachen Composite-Cache `imageData.frames` — sonst faellt eine nur auf einer verdeckten Ebene liegende Kachel heraus — und **nicht** `imagetable:getLength()`), Wrap am Ende, Auto-Ausblenden ~1,5 s nach der letzten Drehung; danach braucht es wieder eine volle Umdrehung. Die Pipette (kurzer B-Tipp) zeigt kurz „Tile N picked“ in der Bauchbinde. **B + Kurbel vor/zurueck** (Zoomkette / Frame-Verwaltung, AD-019/AD-041) bleibt unveraendert (Tick-basiert).
- Begruendung: Der Hardware-Test zeigte, dass eine volle 360°-Kurbelumdrehung fuer *einen* Ebenen- bzw. Frame-Schritt zu langsam und ueberdrehanfaellig ist, und dass die Kurbel im Tile View sonst brachliegt. „Ein Druck = ein Schritt“ ist praezise. **10. Runde:** derselbe Hardware-Test zeigte, dass der Picker bei *jeder* kleinsten Kurbelbewegung aufpoppte — ein- und auspacken der Kurbel loeste ihn staendig aus; die Volldrehungs-Schwelle (analog `PixelRoom`-Rotation / `SelectionRoom`-Sync-Geste, aber richtungsneutral, weil der Picker keinen Crank-Indikator hat) filtert das. CR-01 bleibt gewahrt: B-Zweig `getCrankTicks(4)`, Ohne-B-Zweig ausschliesslich `getCrankChange()` — nie beide APIs fuer Logik im selben Frame. Der Ohne-B-Zweig nutzt zwei signierte Akkumulatoren nacheinander (`pickerArmDegrees` bis zur Aktivierung, dann `crankAccumDegrees` fuer die 30°-Schritte); `pickerArmDegrees` wird bei Aktivierung, Auto-Ausblendung, jedem B-Halten und `entered()` genullt. `bNavConsumed` (in `bDpadNav` gesetzt, nur in `BButtonDown`/`BButtonUp` zurueckgesetzt) verhindert, dass der B-Release nach B + D-Pad zusaetzlich die Pipette ausloest.
...
## 9.31 AD-043: Pixel-Verschiebung wirkt nur auf die Cursor-Zelle (2-Tile-Streifen)

- Status: umgesetzt (Spec 010 US1, Einschraenkung aus dem Hardware-Test, 2026-09-01)
- Entscheidung: B + Pfeiltaste in der Zoom View verschiebt nicht mehr den gesamten Inhalt der aktiven Ebene, sondern nur die **Zelle unter dem Cursor** plus deren **einen Nachbarn in Schieberichtung**. Beide bilden fuer den Schritt einen 2-Tile-Streifen (32×16 bzw. 16×32), der als Ganzes um 1px verschoben wird: der der Ausgangszelle zugewandte Rand des Nachbarn bekommt den austretenden Streifen, der abgewandte Rand des Nachbarn faellt weg (KEIN Wrap, KEIN Weiterreichen an eine dritte Zelle); die von der Schieberichtung abgewandte Kante der Ausgangszelle wird mit dem ebenenabhaengigen „Nicht-Tinte“-Zustand gefuellt. Am Rasterrand ohne Nachbar aendert sich nur die Ausgangszelle. `LayerModel.shiftTileContent(entry, active1, cellIdx, dir, getTile, registerTile)` ersetzt die entfernte Ganz-Ebenen-Funktion `shiftLayerContent`. `EditorRoom:shiftActiveLayer(direction, cellIdx)` ruft sie synchron und `recompositeCell` fuer die 1-2 geaenderten Zellen (ohne `cellIdx`: die Editor-Cursor-Zelle). Die ZoomRoom reicht die Zelle unter dem **Zoom-Cursor** durch und laesst den Cursor stehen — so schiebt wiederholtes B + Pfeil dieselbe Zelle weiter, der Inhalt wandert Spalte fuer Spalte in den Nachbarn und bleibt dort.
- Begruendung: Zwei Rueckmeldungen aus dem Hardware-Test. (1) *Zu langsam:* die Ganz-Ebenen-Verschiebung kostete gemessen ~192.000 `image:sample()`-Aufrufe + 375 `image.new()` **pro Tastendruck**. (2) *Falscher Umfang:* verschoben werden soll nur die Cursor-Zelle, der Inhalt soll in den Nachbarn wandern und bleiben. Die zweite Einschraenkung loest die erste gleich mit: eine 2-Tile-Operation kostet ~2500 Ops und laeuft synchron je Tastendruck — die zwischenzeitlich erwogene aufgeschobene Materialisierung (`pendingShift`/`flushLayerShift`, erster AD-043-Entwurf) entfaellt komplett. Der 2-Tile-Streifen ist die einzige Lesart, die „nur die Cursor-Zelle verschieben“ **und** „Inhalt wandert in den Nachbarn und bleibt“ zugleich erfuellt.
- Konsequenz: Lokal + billig + synchron. `pendingShift`/`flushLayerShift`/`buildZoomContext`-Synthese/`BButtonUp`/alle Flush-Aufrufstellen des ersten Entwurfs entfallen ersatzlos. `LayerModel.shiftLayerContent` + `decodeLayerGrid`/`materializeShiftedGrid`/`imageFromGrid` sind entfernt; die Ganz-Ebenen-Wrap-Semantik (spec.md Edge Case „Pixel Shift Beyond Boundaries“, FR-005) entfaellt — an der Rasterkante geht Inhalt bewusst verloren. `newMockImage` (Testharness) hat weiterhin eine `copy()`-Methode. Headless: die 5 Ganz-Screen-/Deferral-Testabschnitte durch per-Tile-Tests ersetzt (nur Cursor-Zelle + 1 Nachbar; Inhalt wandert + bleibt; Akkumulation ueber 2 Druecke; alle 4 Richtungen; Rasterrand ohne Nachbar; obere Ebene → `absent`; `EditorRoom:shiftActiveLayer(dir, cellIdx)`; ZoomRoom reicht die Cursor-Zelle durch). 394 Assertions gruen, `pdc` sauber, buildNumber 30. Geraete-Messung bleibt offen (Phase 7 T053/T054). Details in [ADR-043](adr/ADR-043-Pixel-Verschiebung-pro-Tile.md).

## 9.32 AD-044: Schuettel-Erkennung als Eigenlogik auf `playdate.readAccelerometer` (Spec 011)

- Status: umgesetzt (Spec 011, US1); `T/W/R`-Endwerte offen bis zum Hardware-Test (Spec Open #4)
- Entscheidung: Der Undo-Dialog wird durch **einmaliges Links-Rechts-Schuetteln** ausgeloest. Das SDK hat **kein** Shake-/Gesten-Ereignis (`readAccelerometer` liefert nur rohe `(x,y,z)` in g), der Sensor war im Projekt bisher **ungenutzt** ⇒ neue Plattformfaehigkeit + begruendete SDK-Abweichung (Constitution I). `ShakeDetector.new(opts?)` → `d:feed(x,y,z,nowMs) -> bool` / `d:reset()`: X-Achsen-Zustandsautomat — Peak `> +T` und Peak `< −T` innerhalb `W` ms → Kante, danach `R` ms Refraktaersperre. Startparameter `T = 0.85 g`, `W = 500 ms`, `R = 1200 ms`. `playdate.startAccelerometer()` in `entered()` der drei Editier-Views (idempotent), `stopAccelerometer()` beim Ruecksprung zu `SelectionRoom`; in der `FrameManagementView` **nicht** ausgewertet (B durch Halte-Geste belegt, R6). Jeder Editier-View liest einmal pro `update()` `readAccelerometer()` und reicht das Sample an `EditorRoom:onShakeSample(x,y,z,commitAndReturn?)`; `EditorRoom` besitzt die eine `ShakeDetector`-Instanz.
- Begruendung: Nur die **Richtungssequenz** „links und rechts" unterscheidet die bewusste Geste von „geruettelt"/Gehen (Fehlalarm, SC-006). `nowMs` wird hereingereicht → `feed` ist eine reine Funktion des Sample-Stroms, mit Mock-Uhr Frame-fuer-Frame headless testbar (kein `playdate.ui`-Ja/Nein-Dialog vorhanden; Menue-Slots + Tasten in den Editier-Views alle belegt). Sensor nur in den Editier-Views → Batterie / Zoom-View-FPS geschont.
- Konsequenz: SDK-frei, headless voll abgedeckt (V12–V16). `arc42/02` fuehrt das Accelerometer jetzt als Eingabefaehigkeit. **Offen**: `T/W/R` am Geraet justieren (≥ 9/10 bewusste Bewegungen, 2 min ohne Fehlalarm), Endwerte in ADR-044 nachtragen, Spec Open #4 schliessen; die Justierung aendert `Source/ShakeDetector.lua` → eigener `buildNumber`-+1-/`pdc`-Zyklus. Risiko R-27/R-28 (Kap. 11). Details in [ADR-044](adr/ADR-044-Schuettel-Erkennung-Accelerometer.md).

## 9.33 AD-045: Undo-Modell — 3 Schritte, sitzungslokal, Voll-Snapshot des Pre-Zustands (Spec 011)

- Status: umgesetzt (Spec 011, US1/US2)
- Entscheidung: `UndoHistory` (Ringpuffer `MAX = 3`, FIFO) haelt Pre-Zustands-Snapshots fuer **genau vier** riskante Operationen — Clear Screen (nur aktive Ebene), Frame loeschen (`FrameManagementView.deleteMarked`, **nicht** der tote `EditorRoom.deleteCurrentFrame`), 90°-Rotation, Pixel-Verschiebung (ADR-043). Feingranulares Malen zaehlt **nicht** (2 Clarifications). `content`-Eintrag: je betroffener Zelle `{prevPosIndex, prevImage}`; `deleteFrame`-Eintrag: tiefe Kopie `frameLayers[index]` + `frames[index]`-Kopie (oder `nil`). Zwei `apply`-Pfade (`applyContentEntry`/`applyDeleteFrameEntry`), Anwendung **nur** in `EditorRoom:undoLast()`. `peekValid(imageData) -> entry, reason` sortiert **vor** dem Dialog nicht-anwendbare Eintraege aus (fehlender Ziel-Frame → FR-006; `deleteFrame` bei 12 Frames → `reason = "frame-limit"`, FR-007). `resolvePrevIndex` bevorzugt den index-stabilen `prevPosIndex` (die Imagetable waechst nur, nummeriert nie um — `ImageStoreCodec.newSaveOperation` ist reine Transformation), `prevImage` ist nur das Sicherheitsnetz. Kein Redo, keine Persistenz (`frames.json` bleibt `"1.1"`).
- Begruendung: Inverse Deltas brechen bei geteilten Tiles / Umnummerierung; der Bild-Referenz-Weg konsultiert nie blind einen alten Index → FR-008 („Speichern leert den Verlauf nicht") bleibt widerspruchsfrei, Constitution II bleibt PASS. „B-Halte-Run = ein Eintrag" (`coalesceTarget` + `runOpen`, `endShiftRun` bei `BButtonUp`/Raumwechsel/Commit): jeder 1-px-Druck als eigener Eintrag wuerde den 3er-Puffer sofort fuellen. 12-Grenze in `peekValid` statt in `undoLast`: FR-012 verlangt einen Dialog nur bei garantiertem Erfolg (kein „rejected" nach dem A-Druck). RAM worst case < ~100 KB (3 Eintraege).
- Konsequenz: `EditorRoom:clearUndoHistory()` (→ `undoHistory:clear()` + `shakeDetector:reset()` + `shiftRun = nil`) in `handleLoadSuccess` + allen `SelectionRoom`-Wechseln → SC-008 (Verlauf nach Bildwechsel garantiert leer). **Zwei Implementierungs-Fallen festgehalten** (headless-verifiziert): (1) `shiftRun` muss in **allen** Load-/Save-Pfaden auf `nil` — sonst koalesziert ein Shift im neuen Bild in einen Eintrag des alten; (2) bedingungsloses `registerTile(prevImage)` erzeugt falsche Indizes → `resolvePrevIndex` bevorzugt `prevPosIndex`. `UndoPrompt.currentLabel()`-Accessor + Accelerometer-Mock fuer die Tests. 494 Assertions gruen, `pdc` sauber, buildNumber 32. Details in [ADR-045](adr/ADR-045-Undo-Modell-3-Schritt-sitzungslokal.md).

## 9.34 AD-046: Modaler Undo-Dialog — ein Modul, vollstaendige Modalitaet, Commit vor Label (Spec 011)

- Status: umgesetzt (Spec 011, US1/US3)
- Entscheidung: `UndoPrompt` als **ein** Singleton-Modul nach dem `SelectionRoom.confirmingDelete`-Muster (`open`/`isOpen`/`handleA`/`handleB`/`draw`/`reset` + `currentLabel`). `open` ist No-op, wenn bereits offen (FR-015). `handleA` schliesst **erst**, ruft **dann** `onConfirm` genau einmal. Jeder der drei Editier-Views ruft `UndoPrompt.draw()` am Ende von `draw()` (Redraw-Bedingung `… or UndoPrompt.isOpen()`) und gated **jeden** Input-Callback + Crank-Block auf `isOpen()` (A → `handleA`, B → `handleB`, sonst frueh-`return`; FR-013). `EditorRoom:undoRequest(commitAndReturn?)` fuehrt `commitAndReturn()` **vor** `peekValid` aus (aus Zoom-/Pixel-View: offene Edits committen + zurueck Tile View), damit das Label zur Aktion passt, die „(A) Ja" anwendet.
- Begruendung: Constitution IV (bewaehrtes Muster wiederverwenden), FR-013/FR-015 an **einer** testbaren Stelle statt drei `confirmingUndo`-Kopien. Commit-first: der Rotation-Eintrag entsteht erst beim Commit (`ZoomRoom:setNewTile` → `recordRotation`) — waehlte `undoRequest` das Label davor, zeigte ein Schuetteln im Pixel-View das Label des aelteren Eintrags, waehrend „(A) Ja" die committete Rotation zuruecknimmt (Label ≠ Wirkung, FR-016-Bruch). Der Dialog erscheint dann im Tile View, wo das Undo-Ergebnis ohnehin sichtbar wird.
- Konsequenz: FR-013/FR-015/FR-016 zentral; V17–V25 headless gruen. **Bewusst in Kauf genommen**: ein Schuetteln im Zoom-/Pixel-View **ohne** vorhandenen Undo committet die offenen Edits + wechselt in den Tile View, bevor „Nothing to undo" erscheint (bewusste Geste, Commit nicht destruktiv). **Einzeilige Abweichung von research.md R7**: dort committet der `onConfirm`-Callback; umgesetzt ist Commit in `undoRequest` vor `peekValid` — Wirkung identisch, nur Reihenfolge Commit ↔ Dialog getauscht. Details in [ADR-046](adr/ADR-046-Modaler-Undo-Dialog.md).

## 9.35 AD-047: CR-01 praezisiert — beide Crank-Lese-APIs jeden Frame entleeren (Spec 011, Code-Review)

- Status: umgesetzt (Spec 011, Code-Review-Nachbesserung Batch 2, Findings F1/F2)
- Entscheidung: CR-01/PR-01 heisst jetzt „pro `update()` **steuert genau eine** Crank-Lese-API die Logik (B-Zweig `getCrankTicks(4)`, Ohne-B-Zweig `getCrankChange()`), **beide** werden aber jeden Frame **einmal gelesen** und der nicht genutzte Wert verworfen". `EditorRoom:handleCrank()` und `PixelRoom:update()` lesen `crankTicks`+`crankChange` am Block-Anfang; bei offenem Undo-Dialog und in den Load-/Save-Zweigen des `EditorRoom:update()` werden beide APIs ebenfalls nur zum Drain gelesen. `ZoomRoom` war schon konform.
- Begruendung: `getCrankTicks(n)` ist zustandsbehaftet („Ticks seit dem letzten Aufruf"). Wird die API waehrend einer laengeren Picker-/Rotationsdrehung nie gelesen, entlaedt der erste B-Frame den kompletten Rueckstand als Phantom-Zoom bzw. Sprung in die Frame-Verwaltung; bei offenem Dialog stauten beide Zaehler. Die „nur eine lesen"-Regel schuetzt vor **Signal-Aufteilung** einer laufenden Geste — die passiert nur, wenn beide Werte **ausgewertet** werden, nicht wenn einer verworfen wird (`getCrankChange`/`getCrankTicks` fuehren getrennte SDK-Basiswerte).
- Konsequenz: `Source/EditorRoom.lua`, `Source/PixelRoom.lua` Crank-Block + Kommentare; CR-01-Formulierungen in arc42 05/06/08 und Risiko R-22 (arc42 11) nachgezogen. Headless: Mock-Aufrufzaehler `crankTicksReadCount`/`crankChangeReadCount`; neue Abschnitte pruefen fuer EditorRoom **und** PixelRoom, dass jeder Zweig (B / ohne B / offener Dialog) beide Zaehler erhoeht; die CR-01/PR-01-Regressionstests (Zoomkette loest weiterhin aus) bleiben gruen. buildNumber 33 → 34, `pdc` sauber, 535 Assertions. Details in [ADR-047](adr/ADR-047-CR-01-Crank-Zaehler-jeden-Frame-entleeren.md).

## 9.36 AD-048: Frame-Verwaltung als dauerhafter Room (Spec 010, 8./9. Runde)

- Status: umgesetzt (Spec 010, 8. Runde aus dem Hardware-Test, 9. Runde aus `/speckit-clarify`)
- Entscheidung: `FrameManagementView` ist ein regulaerer Room im `switchRoom`-Rotationsschema statt eines „B halten"-Modals. Eintritt unveraendert (B + Kurbel rueckwaerts aus dem Tile View, `EditorRoom:handleCrank`), **Verlassen mit B + Kurbel vorwaerts** — ausgewertet erst, nachdem B seit `entered()` **einmal losgelassen** wurde (`bReleasedSinceEnter`); der Room liest pro `update()` nur `getCrankTicks(4)` (CR-01/AD-047) in einen `crankAccu`, der bei nicht gehaltenem B auf 0 faellt. Frames als `playdate.ui.gridview`-Thumbnail-Raster (3 Spalten, `thumbCache` je Position); **A ist ein Markieren/Aufheben-Umschalter** (loescht nie); D-Pad bei markiertem Frame verschiebt ihn in der Sequenz (Links/Rechts = ±1, Hoch/Runter = NUM_COLS als **sequenzielle Nachbar-Swaps**, je Schritt ein `EditorRoom:onFramesReindexed({swapped})`). Loeschen + Duplizieren liegen im **System-Menue**: „delete frame" oeffnet den A/B-Bestaetigungsdialog (wiederverwendet aus `SelectionRoom.confirmingDelete`/`drawConfirmDeleteDialog`), wirkt auf den Cursor-Frame, abgelehnt bei nur 1 Frame; „duplicate frame" fuegt eine tiefe Kopie an `cursor+1` ein (`onFramesReindexed({inserted})`), abgelehnt bei 12 Frames.
- Begruendung: Der „B halten"-Modal war fuer echtes Umsortieren unbequem und hatte eine dokumentierte B-Timing-Sackgasse (Commit `c2cbb6f`). Ein dauerhafter Room mit dem bewaehrten `SelectionRoom`-Raster + -Steuerung fuegt kein neues Paradigma hinzu (Constitution IV) und entfernt Zustand (`bWasHeld`, das B-Loslassen-Exit-Automat). Die Arming-Bedingung macht Eintritt/Verlassen symmetrisch, ohne auf Kurbel-Dezeleration zu spekulieren (headless-testbar). Reorder als Folge von Nachbar-Swaps haelt die Spec-011-`{swapped}`-Payload gueltig — kein neues Reindex-Format fuer die Umordnung.
- Konsequenz: `Source/FrameManagementView.lua` neu geschrieben (`entered`/`update`/`inputHandler`/`draw`/`drawCell` + `moveCursor`/`moveMarked`/`pressA`/`onMenuDelete`/`confirmDelete`/`cancelDelete`/`onMenuDuplicate`/`buildThumbnails`); `Source/UndoHistory.lua` `remapFrames` bekommt den `{inserted}`-Fall (Spiegel von `{removed}`: Eintraege mit Index `>= i` +1, nichts verworfen); `Source/EditorRoom.lua` `onFramesReindexed` nimmt `{inserted}` (Pass-through). `main.lua`-Verdrahtung unveraendert. Die Schuettel-Geste bleibt hier **inaktiv** — Spec 011 FR-010 schliesst die Frame-Verwaltung namentlich aus; der Room startet den Accelerometer nicht (der R6-Grund „B ist gehalten" ist mit dem persistenten Room hinfaellig, das Ergebnis nicht). Headless: `FrameManagementView`-Abschnitte neu (Toggle, Raster-Nav, Hoch/Runter-Dekomposition = 3 `{swapped}`, Menue-Loeschen mit Bestaetigung, Menue-Duplizieren, 12er-Grenze, armierter Exit); V9/V23b/F4/V21 + `remapFrames({inserted})` gruen. buildNumber 39 → 40, `pdc` sauber. Details in [ADR-048](adr/ADR-048-Frame-Verwaltung-persistenter-Room.md).

## 9.37 AD-049: Konsolidierte Tile-View-Overlay-Leiste (Spec 010, 8. Runde)

- Status: umgesetzt (Spec 010, 8. Runde aus dem Hardware-Test)
- Entscheidung: Frame/Ebenen-Label (FR-015), Tile-Picker-Filmstreifen (FR-025), „Tile N picked"-Toast (FR-027) und Statustexte teilen sich EINE Leiste in der dem Tile-Cursor **abgewandten** Bildschirmzone. Horizontale Haelfte weiter nach `cursor.x` (Spec 006 FR-003), vertikaler Rand nach `cursor.y`: `overlayAnchor(cursorY, GRID_ROWS)` → `"bottom"` fuer die obere Cursor-Haelfte, sonst `"top"` (Gleichstand → `"bottom"`). `Bauchbinde:draw(lines, side, vAnchor, w, h)` zeichnet mehrzeilig (Label + Status als zwei Bandzeilen); `drawBottom(text, side, w, h)` bleibt signaturgleicher Einzeiler-Wrapper. Der Tile-Picker liegt bei Sichtbarkeit in derselben Zone (`drawTilePickerOverlay(vAnchor)`), **nicht mehr bildschirmmittig**; ist zugleich Status zu zeigen, weicht er auf den Gegen-Anker aus. Der Undo-Dialog (Spec 011, `UndoPrompt`) bleibt eine **eigene, koordinierte Schicht** darueber.
- Begruendung: Der zentrierte Tile-Picker verdeckte Bild und Cursor (Hardware-Test); Label und Statuszeile kollidierten unten-links, wenn der Cursor rechts stand. Eine cursorabgewandte Leiste folgt der schon fuer die Bauchbinde etablierten Spec-006-FR-003-Konvention, nur zusaetzlich vertikal. Den Undo-Dialog getrennt zu lassen respektiert Spec 011 FR-013 (volle Modalitaet) und vermeidet, Spec 011 wieder zu oeffnen.
- Konsequenz: `Source/Bauchbinde.lua` (`draw` neu, `drawBottom` Wrapper); `Source/EditorRoom.lua` reine Helfer `overlayAnchor`/`overlayRegionRect`/`cursorCellRect` (headless via `EditorRoom.*`), `draw()` komponiert eine Zeilenliste + einen Picker-Aufruf, der separate fixe `drawBottom(statusMessage, "left", …)` entfaellt. Headless: neue SC-008-Sektion (Leiste schneidet fuer jede Cursorzeile die Cursor-Zelle nie; Picker/Status ueberzeichnen sich nicht) + `drawBottom`-4-Arg-Kompatibilitaet; bestehende Bauchbinde-Tests unveraendert gruen. buildNumber 37 → 39, `pdc` sauber. Details in [ADR-049](adr/ADR-049-Konsolidierte-Overlay-Leiste.md).
