...
- Konsequenz: `PixelRoom.buildTileImage` startet den Canvas in der Off-State-Farbe; `PixelRoom:setCurrentTile(tile, idx, offStateCode)` und `ZoomRoom`/`EditorRoom.buildZoomContext` (`activeLayerIsBase`) reichen den Zustand durch. Der Radierer (A auf Tinte) auf Ebene 2/3 fuehrt zu „durchsichtig“, nicht zu opak-weiss. Details in [ADR-040](adr/ADR-040-Pixel-Transparenz-im-Tile.md).
